...
RETURNING id, tenant_id, user_id, run_id, body, sealed_at, sealed_by, seal_reason, created_at, updated_at
`
...
func (q *Queries) AddManifestEvidence(ctx context.Context, arg AddManifestEvidenceParams) (RunManifest, error) {
...
func (q *Queries) CreateManifestCorrection(ctx context.Context, arg CreateManifestCorrectionParams) (RunManifestCorrection, error) {
...
SELECT id, tenant_id, user_id, run_id, body, sealed_at, sealed_by, seal_reason, created_at, updated_at FROM run_manifests WHERE run_id = $1 AND tenant_id = $2
`
...
func (q *Queries) GetManifest(ctx context.Context, arg GetManifestParams) (RunManifest, error) {
	row := q.db.QueryRowContext(ctx, getManifest, arg.RunID, arg.TenantID)
	var i RunManifest
	err := row.Scan(
		&i.ID,
		&i.TenantID,
...
SELECT id, tenant_id, user_id, run_id, body, sealed_at, sealed_by, seal_reason, created_at, updated_at FROM run_manifests WHERE run_id = $1 AND tenant_id = $2 FOR UPDATE
`
...
func (q *Queries) GetManifestForUpdate(ctx context.Context, arg GetManifestForUpdateParams) (RunManifest, error) {
	row := q.db.QueryRowContext(ctx, getManifestForUpdate, arg.RunID, arg.TenantID)
	var i RunManifest
	err := row.Scan(
		&i.ID,
		&i.TenantID,
...
func (q *Queries) InitManifest(ctx context.Context, arg InitManifestParams) (RunManifest, error) {
...
SELECT id, tenant_id, user_id, manifest_id, body, reason, created_at FROM run_manifest_corrections
...
func (q *Queries) LatestManifestCorrection(ctx context.Context, arg LatestManifestCorrectionParams) (RunManifestCorrection, error) {
...
func (q *Queries) ListManifestCorrections(ctx context.Context, arg ListManifestCorrectionsParams) ([]RunManifestCorrection, error) {
	rows, err := q.db.QueryContext(ctx, listManifestCorrections, arg.ManifestID, arg.TenantID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []RunManifestCorrection
	for rows.Next() {
		var i RunManifestCorrection
		if err := rows.Scan(
			&i.ID,
...
func (q *Queries) SealManifest(ctx context.Context, arg SealManifestParams) (RunManifest, error) {
