...
func (runner *Runner) recordStageEvidence(
	...
	if err := runner.mfst.AddEvidence(ctx, run.task.TenantID, run.task.UserID, run.task.ID, patch); err != nil {
...
func (runner *Runner) recordInitialEvidence(
	...
	if err := runner.mfst.AddEvidence(ctx, task.TenantID, task.UserID, task.ID, patch); err != nil {
